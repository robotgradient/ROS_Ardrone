...
void arCallback(const ar_pose::ARMarker & msg)
{
 ...
 if(toggle==1 && giro==0)
	...
	if(y_current>0){velx=-0.1*y_current+Kd*dy;}
	else if(y_current<0){velx=-0.1*y_current+Kd*dy;}
	else{velx=0;}
	if(x_current>0){vely=-0.1*x_current+Kd*dx;}
	else if(x_current<0){vely=-0.1*x_current+Kd*dx;}
	else{vely=0;}
	if(z_current>1){velz=-0.1*z_current*z_current;}
	else if(z_current<0.8){velz=0.1*z_current*z_current;}
	else{velz=0;}
	
	if(yaw>0){angz=-0.5*yaw;}
	else if(yaw<0){angz=-0.5*yaw;}
	else{angz=0;}
	
	if(velx>0.08){velx= 0.08;}
	else if(velx<-0.08){velx=-0.08;}
	...
	ROS_INFO("STABILIZING velocities:vx=%f,vy=%f,vz=%f,angz=%f,position:x=%f,y=%f,z=%f,yaw=%f",velx,vely,velz,angz,x_current, y_current,z_current,yaw);	
		...
		
	{
		...
		vel_pub.publish(velmsg);
		ROS_INFO("STABILIZING AND SPINING");
	}
else if(toggle==0 && giro==1 && z_current>1.5 && pitch>-0.1 && pitch<0.1 )
	{ros::Duration(0.5).sleep();
	giro=2;
	ROS_INFO("MARKER FOUND");
	}
else if(toggle==0 && giro==3)
	{
	...
	ROS_INFO("Towards marker2 distance marker 2:%f,count %d",z_current,count);}
	else if(z_current<0.7 ){velx=-0.1*z_current*z_current;}
	else {velx=0;
		if(count>20){readyforland=1;// PREPARA EL LANDING
...
{
  ros::init(argc, argv, "arsuscriber2");
  ros::NodeHandle n;
  ...
  while(ros::ok()){
	...
	if(giro==2){
		...
		giro_pub.publish(msggiro);
		  ROS_INFO("Out of Turning proccess %f",msggiro.data);}
	
	if(readyforland==1 && landonetime==0)

		{ROS_INFO("LAND ORDER'S");
		landonetime=1;
		}//if
